...
function initializeSchema() {
  const schema = `
    ...
    -- Daily games: aggregate progress per user (drives the streak leaderboard)
    ...
    -- Daily games: one row per (user, game, day) — prevents double rewards
  ...
  db.exec(schema);
  ensureCampusLocationColumns();
}

function ensureCampusLocationColumns() {
  const columns = db.prepare('PRAGMA table_info(campus_locations)').all();
  ...
  if (!columnNames.has('hours')) {
    db.exec('ALTER TABLE campus_locations ADD COLUMN hours TEXT');
  }
}
